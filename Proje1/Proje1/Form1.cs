...
namespace Proje1
{
    public partial class Form1 : Form
    {
        ...
        void FormulleYerlestir(string[] dizi1, string[] dizi2)
        {
            ...
            for (int j = 1; j < dizi2.Length + 2; j++)
            {
                for (int i = 1; i < dizi1.Length + 2; i++)
                {
                    ...
                    else if (i - 1 >= 1 && j - 1 >= 1)  //i ve j 1'den büyük olma durumu
                    {
                        ...
                        int a = Convert.ToInt32(dataGridView1.Rows[i - 1].Cells[j - 1].Value); //1. formül 
                        formul1 = karsilastirma + a;
                        int b = Convert.ToInt32(dataGridView1.Rows[i - 1].Cells[j].Value); //2.formül
                        formul2 = gap + b;
                        int c = Convert.ToInt32(dataGridView1.Rows[i].Cells[j - 1].Value); //3.formül
                        formul3 = gap + c;
                    }
                    else if (i - 1 >= 1 && j >= 1) //sadece i-1 1'den büyük olma durumu
                    {
                    ...
                    else if (i >= 1 && j - 1 >= 1) //sadece j-1 1'den büyük olma durumu
                    {
        ...
        void TabloBoya(string[] bdizin, string[] idizin)
        {
            ...
            int baslangicNoktasi = Convert.ToInt32(dataGridView1.Rows[j].Cells[i].Value);
            dataGridView1.Rows[j].Cells[i].Style.BackColor = Color.Cyan;
            int sifirNoktasi = Convert.ToInt32(dataGridView1.Rows[1].Cells[1].Value);
            dataGridView1.Rows[1].Cells[1].Style.BackColor = Color.Cyan;
            ...
            while (i > 1 && j > 1)
            {
                ...
                MMKarsilastir = DizileriKarsilastir(j - 1, i - 1);

                if (MMKarsilastir > 0) //eðer match varsa çapraza
                {
                    j = j - 1;
                    i = i - 1;                   
                    dataGridView1.Rows[j].Cells[i].Style.BackColor = Color.Cyan;
                }
                else if (MMKarsilastir < 0) //yoksa büyüklüðe göre
                {
